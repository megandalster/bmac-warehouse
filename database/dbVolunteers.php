<?php
/*
 * Copyright 2014 by Student 1, Student 2, Student 3, and Allen
...
/**
 * dbVolunteers class for Homerestore
 * @author Allen Tucker
 * @version May 15, 2014
 */
...
function create_dbVolunteers(){
	...
	$result = mysql_query("CREATE TABLE dbVolunteers (id TEXT NOT NULL, last_name TEXT, first_name TEXT, address TEXT, city TEXT, state TEXT, zip TEXT, 
							phone1 VARCHAR(12) NOT NULL, phone2 VARCHAR(12), email TEXT, type TEXT, status TEXT, license_no TEXT, license_state TEXT, 
							license_expdate TEXT, availability TEXT, birthday TEXT, start_date TEXT, notes TEXT, password TEXT)");
	mysql_close();
	if(!$result){
...
function retrieve_dbVolunteers($id){
	...
	$theVol = new Volunteer($result_row['last_name'], $result_row['first_name'], $result_row['address'], $result_row['city'], $result_row['state'],
							...
							$result_row['license_no'], $result_row['license_state'], $result_row['license_expdate'],
							$result_row['availability'], $result_row['birthday'], $result_row['start_date'], $result_row['notes'], $result_row['password']);
	mysql_close();
	return $theVol;
}
...
function getonlythose_dbVolunteers($type, $status, $name, $availability) {
	connect();
	$query = "SELECT * FROM dbVolunteers WHERE type LIKE '%".$type."%'" . 
